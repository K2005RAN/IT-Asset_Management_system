...
import { Plus, User, HardDrive, Calendar, Edit2, Trash2, X, Link2, Building2, ChevronDown, Search, Layers } from 'lucide-react';

// Premium Modern Dropdown Component with Case-Insensitive String & Object Handling
const ModernSelect = ({ label, icon: Icon, options = [], value, onChange, placeholder, disabled, hintText }) => {
    ...
    const filteredOptions = safeOptions.filter(opt => {
        if (!opt) return false;
        const targetString = typeof opt === 'string' ? opt : `${opt.label || ''} ${opt.sublabel || ''}`;
        return targetString.toLowerCase().includes(search.toLowerCase());
    });

    const selectedOption = safeOptions.find(opt => {
        if (!opt) return false;
        return (typeof opt === 'string' ? opt : opt.value) === value;
    });
...
const AssignedAssets = () => {
    ...
    // 🌟 Allocation type toggle state setup
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            if (assignmentsRes.data) setAssignedAssets(assignmentsRes.data);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = { userId: finalUserId, assetId, assignmentDate };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this asset assignment log record?')) {
            try {
        ...
        <div className="bg-[#0b0f19] rounded-2xl border border-slate-800/60 shadow-[0_32px_64px_rgba(0,0,0,0.5)] overflow-hidden font-sans text-slate-300">
            ...
            <div className="overflow-x-auto">
                {assignedAssets.length === 0 ? (
                    <div className="p-16 text-center text-sm font-medium text-slate-500 font-sans tracking-wide">
                        No active device allocations recorded inside backend clusters.
                    </div>
                ) : (
                    <table className="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr className="bg-slate-950/40 text-slate-400 font-semibold border-b border-slate-800/60 font-mono text-xs uppercase tracking-wider">
                                <th className="py-4 px-6">Assignee Employee Details</th>
                                <th className="py-4 px-6">Device Information Details</th>
                                <th className="py-4 px-6">Date Assigned</th>
                                    ...
                                    <tr key={assignment._id} className="hover:bg-slate-900/30 transition-colors duration-150 group">
                                            ...
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    <div className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors">
                                                        {isStockRow ? "📥 Checked In: Returned to Stock" : (assignment.assignedToName || "Corporate Employee")}
                                                    </div>
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            <div className="inline-flex items-center justify-end space-x-1">
                                                <button onClick={() => handleEdit(assignment)} className="p-2 text-slate-400 hover:text-slate-200 hover:bg-slate-800/60 rounded-xl transition-all" title="Edit Properties">
                                                    <Edit2 className="w-4 h-4" />
                                                </button>
                                                <button onClick={() => handleDelete(assignment._id)} className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all" title="Wipe Log Row">
            ...
            {modalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-md p-4 transition-all duration-300">
                    <div className="bg-[#0b0f19] border border-slate-800 shadow-[0_30px_80px_rgba(0,0,0,0.8)] w-full max-w-md rounded-2xl overflow-hidden relative transform transition-all duration-300">
                        
                        <div className="absolute top-0 right-0 h-40 w-40 bg-gradient-to-bl from-emerald-500/10 to-transparent rounded-bl-full pointer-events-none"></div>
                        ...
                        <div className="px-6 py-5 border-b border-slate-800/60 flex items-center justify-between bg-slate-900/20 relative z-10">
                            <h4 className="text-base font-semibold text-slate-100 flex items-center gap-2.5 tracking-tight">
                                ...
                                {editMode ? 'Modify Hardware Allocation Log' : 'Provision Hardware Unit'}
                            </h4>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5 relative z-10">
                            ...
                            {/* 🌟 STEP 1 DROPDOWN: Fixed dropdown forces selection choice directly */}
                            <ModernSelect 
                            ...
                            {/* 🌟 WORKFORCE SELECTION CONDITIONAL PASS */}
                            {assignmentType === 'USER' ? (
                                <>
                                    <ModernSelect 
                            ...
                            {/* STEP 5/3: Calendar Timestamp Picker */}
                            <div className="space-y-2">
                                <label htmlFor="assignmentDate" className="flex items-center space-x-2 text-xs font-semibold tracking-wide text-slate-400">
                                    ...
                                    <span>{assignmentType === 'STOCK' ? "Step 3: Return Effective Date" : "Step 5: Assignment Activation Date"}</span>
                                </label>
                                <input
                                    id="assignmentDate" 
                                    name="assignmentDate" 
